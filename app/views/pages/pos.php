            ...
            <main class="flex-1 px-4 py-7 sm:px-8">
                <div class="mx-auto max-w-350">
                    <div class="mb-7 flex items-end justify-between">
                        <div>
                            <p class="mb-1 text-sm font-semibold text-[#f7941d]">Cashier workspace</p>
                            <h2 class="text-3xl font-bold tracking-tight text-[#071947]">Point of Sale</h2>
                            <p class="mt-2 text-sm text-slate-500">Click a product to add it to the customer order.</p>
                        </div>

                        <span class="rounded-full bg-blue-50 px-4 py-2 text-xs font-bold text-[#123fa5]">Active batch:
                            <?= htmlspecialchars($activeBatch['batch_name'] ?? 'None', ENT_QUOTES, 'UTF-8') ?></span>
                    </div>
                    <?php if ($message): ?>
                    ...
                    <div class="grid gap-6 lg:grid-cols-[1fr_380px]">
                        <section>
                            <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                                <?php foreach ($products as $product): ?>
                                    ...
                                    <button
                                        class="group overflow-hidden rounded-2xl border border-slate-200 bg-white text-left shadow-sm transition hover:-translate-y-1 hover:border-[#123fa5] hover:shadow-lg"
                                        type="button"
                                        data-product='<?= htmlspecialchars(json_encode($productData), ENT_QUOTES, 'UTF-8') ?>'>
                                        <?php if ($product['image_path']): ?><img class="h-36 w-full object-cover"
                                                src="<?= $baseFolder ?>/<?= htmlspecialchars($product['image_path'], ENT_QUOTES, 'UTF-8') ?>"
                                                alt=""><?php else: ?>
                                            <div
                                                class="flex h-36 items-center justify-center bg-blue-50 text-4xl font-bold text-[#123fa5]">
                                                <?= htmlspecialchars(strtoupper(substr($product['name'], 0, 1)), ENT_QUOTES, 'UTF-8') ?>
                                            </div><?php endif; ?>
                                        <div class="p-4">
                                            <h3 class="font-bold text-[#071947]">
                                                <?= htmlspecialchars($product['name'], ENT_QUOTES, 'UTF-8') ?></h3>
                                            <div class="mt-2 flex items-center justify-between"><span
                                                    class="font-bold text-[#f7941d]">₱<?= number_format((float) $product['base_price'], 2) ?></span><span
                                                    class="text-xs text-slate-400"><?= $available ?></span></div>
                                        </div>
                                    </button>
                                <?php endforeach; ?>
                            </div>
                        ...
                        <aside class="h-fit rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                            <div class="mb-4 flex items-center justify-between">
                                <h3 class="text-lg font-bold text-[#071947]">Current order</h3><span
                                    class="rounded-full bg-blue-50 px-2 py-1 text-xs font-bold text-[#123fa5]"
                                    data-cart-count>0 items</span>
                            </div>
                            <div class="space-y-3" data-cart></div>
                            <div class="mt-5 border-t border-slate-100 pt-4">
                                <div class="flex justify-between text-lg font-bold text-[#071947]">
                                    <span>Total</span><span>₱<span data-total>0.00</span></span></div><button
                                    class="mt-4 min-h-12 w-full rounded-xl bg-[#f7941d] px-4 font-bold text-white transition hover:bg-[#d96d00] disabled:cursor-not-allowed disabled:opacity-50"
                                    type="button" data-checkout disabled>Proceed to payment</button>
                            </div>
                        </aside>
                    </div>
                </div>
