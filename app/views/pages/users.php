            ...
            <main class="flex-1 px-4 py-7 sm:px-8">
                <div class="mx-auto max-w-350">
                    <div class="mb-7">
                        ...
                        <h2 class="text-3xl font-bold text-[#071947]">User Management</h2>
                    </div>
                    <?php if ($message): ?>
                        <div
                    ...
                    <div class="grid gap-6 xl:grid-cols-[340px_1fr]">
                        <form class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm" method="post">
                            ...
                            <h3 class="mb-5 text-lg font-bold text-[#071947]">Create account</h3>
                            <label class="mb-4 block text-sm font-semibold text-slate-600">Username<input
                                    class="mt-2 min-h-11 w-full rounded-xl border border-slate-200 px-3" name="username"
                                    required></label>
                            <label class="mb-4 block text-sm font-semibold text-slate-600">Full name<input
                                    class="mt-2 min-h-11 w-full rounded-xl border border-slate-200 px-3"
                                    name="full_name" required></label>
                            <label class="mb-4 block text-sm font-semibold text-slate-600">Role<select
                                    class="mt-2 min-h-11 w-full rounded-xl border border-slate-200 px-3" name="role">
                                    <option value="cashier">Cashier</option>
                                    <option value="officer">Officer</option>
                                    <option value="admin">Admin</option>
                                </select></label>
                            <label class="mb-4 block text-sm font-semibold text-slate-600">Password<input
                                    class="mt-2 min-h-11 w-full rounded-xl border border-slate-200 px-3" type="password"
                                    name="password" required></label>
                            <label class="mb-6 block text-sm font-semibold text-slate-600">Confirm password<input
                                    class="mt-2 min-h-11 w-full rounded-xl border border-slate-200 px-3" type="password"
                                    name="confirm_password" required></label>
                            <button class="min-h-11 w-full rounded-xl bg-[#123fa5] px-4 text-sm font-bold text-white"
                                type="submit">Create account</button>
                        </form>
                        <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                            <div class="border-b border-slate-100 px-6 py-5">
                                <h3 class="font-bold text-[#071947]">Accounts</h3>
                            </div>
                                    ...
                                    <tbody class="divide-y divide-slate-100">
                                        <?php foreach ($users as $user): ?>
                                            <tr>
                                                ...
                                                <td class="px-6 py-4">
                                                    <?= htmlspecialchars($user['full_name'], ENT_QUOTES, 'UTF-8') ?></td>
                                                <td class="px-6 py-4 capitalize">
                                                    <?= htmlspecialchars($user['role'], ENT_QUOTES, 'UTF-8') ?></td>
                                                <td class="px-6 py-4">
                                                    <details>
                                                        <summary class="cursor-pointer font-bold text-[#123fa5]">Edit
                                                        </summary>
                                                        <form class="mt-3 grid max-w-64 gap-2" method="post"><input
                                                                type="hidden" name="action" value="edit_user"><input
                                                                type="hidden" name="user_id"
                                                                value="<?= (int) $user['user_id'] ?>"><input
                                                                class="rounded-lg border p-2" name="username"
                                                                value="<?= htmlspecialchars($user['username'], ENT_QUOTES, 'UTF-8') ?>"
                                                                required><input class="rounded-lg border p-2"
                                                                name="full_name"
                                                                value="<?= htmlspecialchars($user['full_name'], ENT_QUOTES, 'UTF-8') ?>"
                                                                required><select class="rounded-lg border p-2"
                                                                name="role"><?php foreach (['cashier', 'officer', 'admin'] as $role): ?>
                                                                    <option value="<?= $role ?>" <?= $user['role'] === $role ? 'selected' : '' ?>><?= ucfirst($role) ?></option>
                                                                <?php endforeach; ?>
                                                            </select><input class="rounded-lg border p-2" type="password"
                                                                name="password" placeholder="New password (optional)"><input
                                                                class="rounded-lg border p-2" type="password"
                                                                name="confirm_password"
                                                                placeholder="Confirm new password"><button
                                                                class="rounded-lg bg-[#123fa5] px-3 py-2 text-xs font-bold text-white"
                                                                type="submit">Save</button></form>
                                                    </details>
                                                    <form class="mt-2" method="post" data-confirm="delete"><input
                                                            type="hidden" name="action" value="delete_user"><input
                                                            type="hidden" name="user_id"
                                                            value="<?= (int) $user['user_id'] ?>"><button
                                                            class="cursor-pointer text-xs font-bold text-red-600"
